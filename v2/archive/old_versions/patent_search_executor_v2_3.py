...
class PatentSearchExecutor:
    ...
    def _execute_patentfield_search(
        ...
        try:
            ...
            # app_doc_id を優先、なければ pub_id を使用
            # これにより出願番号形式(JP2012040876A)でのマッチングが可能になる
            patent_ids = [
    ...
    def _search_with_adjustment(
        ...
        for attempt in range(max_attempts):
            ...
            if attempt == 0:
                ...
                if use_keywords:
                    strategy = f"拡大 階層+{abs(hierarchy_shift)} + {keyword_concept_level}キーワード (拡大{expand_count}/5回目)"
                else:
                    strategy = f"拡大 階層+{abs(hierarchy_shift)} + キーワードなし (拡大{expand_count}/5回目)"
            ...
            if hits < target_min and expand_count < 5:
                # 拡大: 階層を1つ上げる + キーワード概念レベルを調整
                hierarchy_shift -= 1
                expand_count += 1

                # expand_countに応じてキーワード戦略を変更
                if expand_count <= 2:
                    # 拡大1-2回: ドンピシャキーワード継続
                    keyword_concept_level = 'ドンピシャ'
                    use_keywords = True
                    print(f"  → 次回: 階層+{abs(hierarchy_shift)} + ドンピシャキーワード（拡大{expand_count}/5回目）")
                elif expand_count <= 4:
                    # 拡大3-4回: 上位概念キーワードに切り替え
                    keyword_concept_level = '上位概念'
                    use_keywords = True
                    print(f"  → 次回: 階層+{abs(hierarchy_shift)} + 上位概念キーワード（拡大{expand_count}/5回目）")
                else:
                    # 拡大5回: キーワードなし
                    use_keywords = False
                    print(f"  → 次回: 階層+{abs(hierarchy_shift)} + キーワードなし（拡大{expand_count}/5回目）")
