...
class PatentSearchExecutor:
    ...
    def _build_search_query(
        ...
        if not classification_query:
            print(f"  ⚠ 警告: {axis_element_id}に分類コードなし")
            return ""
    ...
    def _search_with_adjustment(
        ...
        for attempt in range(max_attempts):
            if attempt == 0:
                ...
                if final_hits < target_min:
                    # 拡大：上位概念分類追加
                    concept_level = '上位概念'
                    use_keywords = True
                    strategy = "上位概念分類 + ドンピシャキーワード"
                else:
                    # 縮小：下位概念分類で絞り込み
                    concept_level = '下位概念'
                    use_keywords = True
                    strategy = "下位概念分類 + ドンピシャキーワード"
                ...
                if final_hits < target_min:
                    ...
                    concept_level = '下位概念'
                    use_keywords = True
                    strategy = "下位概念分類 + キーワード（強化）"

            elif attempt == 3:
                if final_hits < target_min:
                    # 拡大：キーワードのみ
                    query = self._build_keyword_query(
                        [axis_element_id] + other_element_ids,
                        concept_level='上位概念',
                        max_keywords=5
                    )
                    strategy = "上位概念キーワードのみ"
                else:
                    # 縮小：キーワード追加で厳密化
                    concept_level = '下位概念'
                    use_keywords = True
                    # キーワード数を増やして絞り込み
                    strategy = "下位概念分類 + キーワード追加"
                ...
                if final_hits < target_min:
                    ...
                    strategy = "ドンピシャキーワードのみ（最終拡大）"
                else:
                    # これ以上縮小不可、現状維持
                    break

            # 検索式構築
            if attempt <= 2 or (attempt == 3 and final_hits >= target_min):
            ...
            if not query:
                print(f"  試行{attempt + 1}: 検索式構築失敗")
                continue

            print(f"\n  試行{attempt + 1}: {strategy}")
            print(f"  検索式: {query[:100]}...")
            ...
            attempts.append({
                'attempt': attempt + 1,
                'strategy': strategy,
                'concept_level': concept_level if attempt <= 3 else 'キーワードのみ',
                'query': query,
                'hits': hits
            })
